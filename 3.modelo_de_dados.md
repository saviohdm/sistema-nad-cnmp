...
Este documento descreve o modelo de dados do Sistema de Proposições do Conselho Nacional do Ministério Público (CNMP). O sistema gerencia o acompanhamento de proposições (determinações e recomendações) emitidas em sede de correição para os 30 ramos do Ministério Público brasileiro.

Sob a ótica de modelagem de domínio, este documento cobre principalmente o **núcleo do domínio** relacionado ao acompanhamento de proposições e, em menor grau, o contexto de correições. Aspectos de autenticação, autorização, dashboard, exportação e notificações são apenas referenciados quando impactam diretamente a estrutura dos dados.
...
- Explicitar decisões conceituais relevantes para o núcleo de domínio

---

## 2. Visão Geral do Modelo

O modelo de dados é composto por **2 entidades principais** e **2 estruturas internas relevantes**:
...
3. **Histórico** - Estrutura interna imutável da Proposição para registrar interações
4. **Rascunho** - Estrutura interna temporária da Proposição para preparação de comprovação

### 2.1.1 Leitura Conceitual do Modelo

Para alinhamento com a modelagem orientada ao domínio:

- **Proposição** é o elemento central do núcleo de negócio e concentra o ciclo de vida da publicação, comprovação e avaliação
- **Correição** organiza institucionalmente as proposições e fornece o contexto de origem
- **Histórico** não é tratado aqui como entidade independente do sistema, mas como uma coleção embarcada e append-only da Proposição
- **Rascunho** também é interno à Proposição e representa um estado transitório, sem autonomia fora dela
- O campo `status` da Proposição representa conceitualmente um **Value Object** chamado `StatusDaProposicao`, embora possa ser persistido como array ou como dois campos separados
- O campo `status` da Correição é **derivado** das proposições vinculadas e deve ser entendido como estado calculado, não como dado primário editável
...
| `status` | Enum | Sim | Status calculado: 'ativo' ou 'inativo'; representa estado derivado das proposições vinculadas |
...
- **RN-COR-05:** O campo `status` é calculado automaticamente:
  ...
  - O status não deve ser alterado manualmente
...
Do ponto de vista conceitual, a **Proposição** é a estrutura central deste modelo, pois é nela que se concentram as invariantes do workflow de negócio: publicação, comprovação, avaliação, republicação, rastreabilidade e encerramento.
...
| `status` | Value Object persistido como Array[Enum] | Sim | Status bidimensional da proposição: [statusProcessual, valoracao] |
...
| `historico` | Array[Object] | Sim | Coleção append-only de interações relevantes da proposição |

#### 3.2.2 Value Object StatusDaProposicao

Conceitualmente, o campo `status` representa um **Value Object** chamado `StatusDaProposicao`.

Na persistência, ele pode ser representado como:

- um **array de 2 elementos**: `[statusProcessual, valoracao]`
- ou dois campos separados: `statusProcessual` e `valoracao`

Essa distinção é importante porque o conceito de domínio não é o array em si, mas a combinação coerente entre:

- o estado do fluxo
- o resultado qualitativo da última avaliação
...
`rascunhos` é uma estrutura **interna** da Proposição. Ela não representa um agregado independente, mas um estado intermediário e editável da futura comprovação.
...
Representa uma interação registrada em uma proposição. Cada entrada no histórico documenta uma publicação, comprovação ou avaliação.

O histórico é uma estrutura **embarcada** na entidade Proposição como um array e deve ser entendido como uma coleção **append-only** do ciclo de vida da proposição, não como uma entidade autônoma do sistema.
...
| `statusAnterior` | Value Object persistido como Array[Enum] | Não | Status da proposição antes da interação (publicacao e avaliacao) |
| `statusNovo` | Value Object persistido como Array[Enum] | Não | Status da proposição após a interação (publicacao e avaliacao) |
...
- **RN-HIST-04:** Para `tipo = 'comprovacao'`:
  - `arquivos` é facultativo e pode estar ausente ou vazio
...
### 4.3 Proposição → Rascunho (1:N lógico, 0..1 ativo)

**Tipo:** One-to-Many Embedded com restrição de unicidade lógica do rascunho ativo

**Cardinalidade:**
- Uma proposição pode ter 0 ou mais rascunhos historicamente persistidos, mas no modelo atual mantém no máximo 1 rascunho ativo
- Cada rascunho pertence a exatamente 1 proposição

**Implementação:**
- Array embarcado `rascunhos` na entidade Proposição
- No estado atual do sistema, a regra operacional é `length ∈ {0,1}`

**Regras:**
- Rascunho não altera o status da proposição
- Rascunho é substituível
- Envio oficial de comprovação consome e limpa o rascunho ativo
...
- **WF-06:** O status da Correição deve ser recalculado após alterações relevantes nas proposições vinculadas
...
CREATE TABLE correicao (
  ...
  status ENUM('ativo', 'inativo') NOT NULL, -- Campo derivado; não editável manualmente
...
CREATE TABLE proposicao (
  ...
  dataPublicacao DATE,
  statusProcessual ENUM('pendente', 'aguardando_comprovacao', 'em_analise', 'encerrada') NOT NULL, -- Parte do VO StatusDaProposicao
  valoracao ENUM('nova', 'finalizada', 'parcial', 'em_andamento', 'prejudicada') NOT NULL, -- Parte do VO StatusDaProposicao
  tags JSON, -- Array de strings
  rascunhos JSON, -- Array de objetos; regra atual: no máximo 1 rascunho ativo
  historico JSON, -- Array append-only de objetos
  FOREIGN KEY (correicaoId) REFERENCES correicao(id) ON DELETE RESTRICT,
...
- O Value Object `StatusDaProposicao` pode ser desmembrado em 2 campos (`statusProcessual` + `valoracao`) para facilitar indexação e consultas
- O campo `status` da correição é derivado e pode ser persistido por conveniência operacional, desde que exista rotina confiável de recálculo
...
- Validar que arquivos anexados existem no storage quando informados
- Validar que `historico` só receba novas entradas
- Validar que `status` da correição não seja alterado manualmente
...
| **Histórico** | Coleção cronológica e append-only de interações relevantes de uma proposição |
