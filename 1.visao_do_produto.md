...
- Formulário de avaliação com:
  - Decisão (radio buttons visuais): finalizada, parcial, em_andamento, prejudicada
  - Parecer detalhado (máx 7.500 caracteres)
- Contador de caracteres em tempo real
- Lógica de transição de status:
  - Finalizada/Prejudicada → status `encerrada` (fim do ciclo)
  - Parcial/Em Andamento → status `pendente` (aguarda republicação)
